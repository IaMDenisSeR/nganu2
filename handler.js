...
global.dfail = (type, m, conn) => {
  let msg = {
    rowner: '*Owner* ```can use This!```',
        owner: '*Bot owner* ```can use This!```',
        mods: '*Moderator* ```can use This!```',
        group: '*This Command* ```work Group chat only!!!```',
        private: '*This Command* ```work Private chat only!!!```',
        admin: '*Admin* ```can use This!```',
        botAdmin: '*Bot make admin* ```to use This!```',
        restrict: '``This featur has disabled by owner!!!```'
  }[type]
