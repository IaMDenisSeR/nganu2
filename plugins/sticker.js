...
let handler = async (m, { conn, args, usedPrefix, text, command }) => {
  ...
  try {
    ...
    if (/image/.test(mime)) {
      ...
      if ((q.msg || q).seconds > 11) return m.reply('```10 Second Maximum.```')
      let img = await q.download()
      ...
      stiker = await sticker(img, false, global.packname, global.author)
    } else if (q.mentionedJid) {
	    let media = await conn.profilePictureUrl(q.mentionedJid[0], 'image')
	    if (!media) throw 'Mention Any Contact'
